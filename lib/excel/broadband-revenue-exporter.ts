...
export type BroadbandRevenueExportInput = {
  report: BroadbandRevenueOverview;
  exportedAt?: Date;
};
...
export async function buildBroadbandRevenueWorkbook(input: BroadbandRevenueExportInput) {
  ...
  workbook.subject = `รายได้ Broadband ${formatBuddhistYear(report.reportYear)}`;

  const sheet = workbook.addWorksheet("รายได้ Broadband", {
  ...
  title.value = "รายได้ Broadband";
  title.font = { name: "Aptos Display", size: 18, bold: true, color: { argb: "FFFFFFFF" } };
  title.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF1E1B4B" } };
  title.alignment = { vertical: "middle", horizontal: "left" };
  sheet.getRow(1).height = 34;

  sheet.mergeCells("A2:J2");
  const subtitle = sheet.getCell("A2");
  subtitle.value = `${report.organization.label} · ${report.service.label} · มกราคม–${monthName} ${formatBuddhistYear(report.reportYear)}`;
  ...
  metadata.forEach(([label, value], index) => {
    ...
    row.getCell(1).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFE0F2F1" } };
  ...
  report.rows.forEach((reportRow, index) => {
    ...
    if (reportRow.level === "group") {
      row.eachCell((cell) => {
        ...
        cell.font = { bold: true, color: { argb: "FF115E59" } };
        cell.border = { top: { style: "double", color: { argb: "FF312E81" } } };
      });
      row.height = 28;
    } else {
      row.eachCell((cell) => {
  ...
  sheet.headerFooter.oddFooter = `รายได้ Broadband ${formatBuddhistYear(report.reportYear)} · หน้า &P / &N`;

  return workbook;
}

export async function downloadBroadbandRevenueWorkbook(input: BroadbandRevenueExportInput) {
  const exportedAt = input.exportedAt ?? new Date();
  const workbook = await buildBroadbandRevenueWorkbook({ ...input, exportedAt });
  const buffer = await workbook.xlsx.writeBuffer();
  const blob = new Blob([buffer], {
    type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
  });
  const filename = `รายได้_Broadband_พศ${input.report.reportYear + 543}_ถึงเดือน${String(input.report.throughMonth).padStart(2, "0")}_${exportTimestamp(exportedAt)}.xlsx`;
