create index if not exists revenue_import_rows_area_report_idx
on public.revenue_import_rows(batch_id, unit_name, section_name);

create or replace function public.get_op_area_overview_report(p_year integer)
returns jsonb
language plpgsql
stable
security definer
...
revoke all on function public.get_op_area_overview_report(integer) from public;
grant execute on function public.get_op_area_overview_report(integer) to authenticated;
