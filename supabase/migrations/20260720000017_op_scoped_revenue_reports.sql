create index if not exists revenue_import_rows_op_scoped_report_idx
on public.revenue_import_rows(owner_id, batch_id, business_group, service_group, unit_name, section_name);
...
begin
  ...
  from (
    values
      ('broadband', 'service_group', '4.Fixed Line & Broadband',
        '4.2.กลุ่มบริการ Internet Retail', 'Internet Retail (Broadband)', 'รายได้ Broadband'),
      ('datacom', 'service_group', '4.Fixed Line & Broadband',
        '4.3.กลุ่มบริการวงจรเช่า (Datacom)', 'กลุ่มบริการวงจรเช่า (Datacom)', 'รายได้ Datacom'),
      ('fixed-line', 'service_group', '4.Fixed Line & Broadband',
        '4.4.บริการโทรศัพท์ประจำที่ (Fixed Line)', 'บริการโทรศัพท์ประจำที่ (Fixed Line)',
        'รายได้ Fixed Line'),
      ('mobile-retail', 'service_group', '3.Mobile',
        '3.2.บริการโทรคมนาคมสื่อสารไร้สาย - กลุ่มค้าปลีก (Retail)',
        'กลุ่มบริการโทรคมนาคมสื่อสารไร้สาย - กลุ่มค้าปลีก (Retail)', 'รายได้ Mobile Retail'),
      ('ict-solution', 'business_group', '6.ICT Solution', null::text,
        'กลุ่มธุรกิจ ICT Solution', 'รายได้ ICT-Solution')
  ) as s(scope_key, scope_level, business_group, service_group, scope_label, report_title)
  ...
  current_rows as materialized (
    ...
    where r.owner_id = v_owner
      ...
      and (
        ...
        or r.service_group = v_service_group
      )
  ...
  matching_targets as materialized (
    ...
    where t.owner_id = v_owner
      ...
      and (
        (v_scope_level = 'business_group' and t.service_group is null)
        or (v_scope_level = 'service_group' and t.service_group = v_service_group)
      )
      and (
